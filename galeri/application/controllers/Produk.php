...
class Produk extends CI_Controller {
	...
	{	
		...
		//ambil data total
		$total =$this->produk_model->total_produk_filter_by_price($lower_price, $upper_price);
		//paginasi Start
		$this->load->library('pagination');
		$this->load->library('pagination');
		
		$config['base_url'] 		= base_url().'produk/filter_by_price/';
		$config['total_rows'] 		= $total->total;
		$config['use_page_numbers']	= TRUE;
		$config['per_page'] 		= 3;
		$config['uri_segment']	 	= 5;
		...
		$config['cur_tag_open'] 	= '<b>';
		$config['cur_tag_close'] 	= '</b>';
		$config['first_url']		= base_url().'/produk/filter_by_price/';
		
		$this->pagination->initialize($config);
		
		// ambil data produknya
		$page 	= ($this->uri->segment(5))? ($this->uri->segment(3)-1) * $config['per_page']:0;
		$produk =$this->produk_model->filter_by_price($lower_price, $upper_price, $config['per_page'], $page);
